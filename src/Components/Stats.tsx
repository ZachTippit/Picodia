...
const Stats = () => {
  ...
  const stats = useMemo(() => {
    ...
    const averageTimeSeconds =
      gamesCompleted > 0 ? (profileStats?.total_completed_time_seconds ?? 0) / gamesCompleted : null;
    const averageLives =
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={cn(
          'relative z-10 w-full max-w-sm rounded-3xl border p-6 text-center shadow-2xl border-gray-300 bg-white text-gray-900',
          ...
          {isPending ? (
            ...
            stats.map((entry) => (
              <div
                key={entry.label}
                className={cn(
                  'flex items-center justify-between rounded-xl px-4 py-3 text-sm font-medium transition-opacity duration-300 bg-gray-100 text-gray-700'
                )}
              >
